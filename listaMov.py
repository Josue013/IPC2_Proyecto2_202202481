...
class listaMov:
    ...
    def completar_esperar(self, dron_buscar, altura, num):
        ...
        for tiempos in range(1, tiempo+1):
            if self.validar_posicion(tiempos, dron_buscar) == False:
                nuevo_movimiento = mov("Esperar", tiempos, dron_buscar,altura, num)
    ...
    def generar_grafica(self, cantidad, name_msg, drones):
        ...
        texto += f"""<TR><TD colspan="{int(cantidad)+1}" border="1" bgcolor="#FF5B0E">"""+name_msg+"""</TD></TR>\n"""
        
        texto += """<TR>\n<TD border="1" bgcolor="red">Tiempo</TD>"""
        for dron in drones:
            texto += f"""<TD border="1" bgcolor="orange">{dron.nombre}</TD>\n"""
        texto += "</TR>"
        ...
        os.system(f'dot -Tpng bb.dot -o {name_msg}.png')
    ...
    def agregar_movimientos_grafica(self):
        ...
        texto = f"""<TR>\n<TD border="1" bgcolor="red">{actual.tipo_dato.tiempo}</TD>\n"""
        fila = False
        while actual != None:
            ...
            if fila == False:
                fila = True
                texto += f"""<TD border="1" bgcolor="orange">{actual.tipo_dato.movimiento}</TD>\n"""
            else:
                texto += f"""<TD border="1" bgcolor="orange">{actual.tipo_dato.movimiento}</TD>\n"""
